...
<body style='min-height: 75rem; padding-top: 3.5rem;'>
  <nav class="navbar navbar-expand-lg fixed-top navbar-dark bg-dark">
      <a class="navbar-brand" href="#">AddressSettings</a>
  ...
  <table style="width:640px" id='data_table' class="table table-light table-bordered table-striped table-hover">
    ...
    <tbody>
      <xsl:apply-templates select="sf:AddressSettings/sf:countriesAndStates" >
      </xsl:apply-templates>
   </tbody>
  </table>
</div>
...
<script>
  $(document).ready( function () {
     ...
     <!-- templates -->
     <xsl:template match="sf:AddressSettings/sf:countriesAndStates">
      <xsl:for-each select="sf:countries">
      <tr>
             <td><xsl:value-of select="sf:active"/></td>
             <td><xsl:value-of select="sf:integrationValue"/></td>
             <td><xsl:value-of select="sf:isoCode"/></td>

             <td><xsl:value-of select="sf:label"/></td>
             <td><xsl:value-of select="sf:orgDefault"/></td>
             <td><xsl:value-of select="sf:standard"/></td>
             <td>
